...
class CreateLeadershipIncomesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('leadership_incomes', function (Blueprint $table) {
            ...
            $table->smallInteger('leadershipid');
            $table->decimal('amount',36,8)->default();
            $table->decimal('remaining',36,8)->default();
            $table->decimal('amt_usdt',36,8)->default();
            $table->decimal('remaining_usdt',36,8)->default(0);
            $table->string('txnDesc',20)->nullable();
