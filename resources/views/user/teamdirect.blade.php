...
<div class="mecha-hud-card">
    ...
    <div class="cyber-toolbar-wrap">
        <div class="toolbar-show-records">
            <span>SHOW</span>
            <select id="pageSizeSelect" class="toolbar-select-styled">
        ...
        <div class="toolbar-search-box">
            ...
            <input type="text" id="memberSearchInput" class="toolbar-search-input" placeholder="Search by User ID, wallet, date...">
        </div>
    </div>

    <!-- Cards Grid Container -->
    <div class="direct-cards-grid" id="cardsGridContainer">
        @php $i = 1; @endphp
        @forelse($direct as $row)
            @php
                ...
                if (method_exists($row, 'levelStatus')) {
                    $levelPct = $row->levelStatus() >= ($row->leveluser ?? 0) ? round($row->levelStatus()) : round($row->leveluser ?? 0);
                    ...
                    $levelPct = $ud->levelStatus() >= ($row->leveluser ?? 0) ? round($ud->levelStatus()) : round($row->leveluser ?? 0);
                } else {
                    $levelPct = round($row->leveluser ?? 0);
                }
                
                $mWallet = $row->walletaddress ?? $row->bep20address ?? '';
            @endphp
            
            <div class="member-cyber-card" 
                 data-search="{{ strtolower(($row->userid ?? '').' '.($mWallet ?? '').' '.($row->doj ?? '').' '.($isActive ? 'active' : 'inactive').' '.($row->shares ?? '').' '.($row->teamtotal ?? '')) }}">
                ...
                <div class="card-stats-2x2">
                    <div class="stat-item-cell">
                        <span class="stat-cell-lbl"><i class="fas fa-coins" style="color:#00FF88;"></i> SELF STAKED</span>
                        <span class="stat-cell-val val-green">${{ number_format((float)($row->shares ?? 0), 2) }}</span>
                    </div>

                    <div class="stat-item-cell">
                        <span class="stat-cell-lbl"><i class="fas fa-chart-line" style="color:#00E5FF;"></i> TURNOVER</span>
                        <span class="stat-cell-val val-cyan">${{ number_format((float)($row->teamtotal ?? 0), 2) }}</span>
                    </div>

                    <div class="stat-item-cell">
                        <span class="stat-cell-lbl"><i class="fas fa-layer-group" style="color:#FFD700;"></i> UNLOCK</span>
                        <span class="stat-cell-val val-gold">{{ $levelPct }}%</span>
                    </div>
        ...
        @empty
            <div class="no-records-card">
                ...
                <div class="no-records-title">No Direct Referrals Yet</div>
                <div class="no-records-sub">Invite members using your referral link or register a new partner to build your direct network team!</div>
                <a href="{{ url('/User/Referral') }}" class="mecha-btn-gold" style="width: auto; height: 32px; padding: 0 14px; margin-top: 4px; font-size: 10px;">
                    <i class="fas fa-qrcode"></i> GET REFERRAL LINK
                </a>
            </div>
        @endforelse
    </div>
    ...
    <div id="noMatchSearchCard" class="no-records-card" style="display: none; margin-bottom: 14px;">
        <div class="no-records-icon" style="color: #64748B; background: rgba(255,255,255,0.05); border-color: rgba(255,255,255,0.1);">
            <i class="fas fa-search"></i>
        </div>
        <div class="no-records-title">No Matching Direct Members</div>
        <div class="no-records-sub">We couldn't find any direct team members matching your search query. Try another keyword.</div>
    </div>

    <!-- Bottom Pagination Bar -->
    <div class="mecha-pagination-bar" id="paginationBar">
        <div class="pagination-info-txt" id="paginationInfoTxt">
            Showing 0 to 0 of 0 entries
        </div>
        <div class="pagination-btns-wrap" id="paginationBtnsWrap">
...
<script>
    ...
    // Reactive Live Search & Pagination Controller
    document.addEventListener('DOMContentLoaded', function () {
        const allCardElements = Array.from(document.querySelectorAll('.member-cyber-card'));
        const searchInput = document.getElementById('memberSearchInput');
        const pageSizeSelect = document.getElementById('pageSizeSelect');
        const noMatchCard = document.getElementById('noMatchSearchCard');
        const paginationInfo = document.getElementById('paginationInfoTxt');
        const paginationBtns = document.getElementById('paginationBtnsWrap');
