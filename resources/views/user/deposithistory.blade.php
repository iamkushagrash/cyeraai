...
<div class="mecha-hud-card">
    <div class="mecha-card-header">
        ...
        <a href="{{ url('/User/Stake') }}" class="mecha-btn-gold" style="width: auto; height: 32px; padding: 0 14px; font-size: 10px;">
            <i class="fas fa-plus"></i> NEW STAKE
        </a>
    </div>

    <div class="cyber-cards-table-container">
        <table id="depositHistoryTable" class="cyber-card-datatable" style="width: 100%;">
            <thead>
                            ...
                            <div class="staking-cyber-card">
                                ...
                                <div class="card-header-row">
                                    ...
                                    <div class="card-status-wrap">
                                        <span class="status-pill {{ $pillClass }}">
                                            <i class="fas {{ $pillClass === 'success' ? 'fa-circle-check' : ($pillClass === 'danger' ? 'fa-circle-xmark' : 'fa-clock') }}"></i>
                                            {{ $row->status }}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="card-metrics-grid">
                                    <div class="metric-item">
                                        ...
                                        <span class="metric-val val-green">${{ number_format((float)$row->amountusdt, 2) }}</span>
                                    </div>
                                    <div class="metric-item">
                                        <span class="metric-lbl">CURRENCY</span>
                                        <span class="metric-val val-gold"><i class="fas fa-coins"></i> {{ strtoupper($row->currency ?? 'USDT') }}</span>
                                    </div>
                                    <div class="metric-item">
                                        <span class="metric-lbl">TIMESTAMP</span>
                                        <span class="metric-val val-muted"><i class="far fa-calendar-alt"></i> {{ $row->created_at }}</span>
                                    </div>
                                    <div class="metric-item">
                                        <span class="metric-lbl">NETWORK</span>
                                        <span class="metric-val val-cyan"><i class="fas fa-link"></i> BSC (BEP20)</span>
                                    </div>
                                </div>
                            </div>
                        </td>
