...
<head>
    ...
    <title>User Withdraw Requests</title>
                ...
                <div class='col-xs-12'>
                    <div class="page-title">
                        ...
                        <div class="pull-left">
                            ...
                            <h1 class="title">Withdraw Requests</h1>
                            ...
                            <div class="row">
                                <div class="col-xs-12">
                                    ...
                                    <div class="form-group" style="margin-top: 10px;">
                                        <form action="/Main/WithdrawRequests" method="POST">
                                        @csrf
                                           <div class="col-lg-3">
                ...
                <div class="col-xs-12">
                    <section class="box over-h">
                        <div class="content-body">    
                            <div class="row">
                                <div class="col-xs-12">
                                    <h4>Total Pending : {{round($sumtotal['amountusdt'],2)}}$ </h4>
                ...
                <div class="col-lg-12">
                    <section class="box has-border-left-3">
                        <header class="panel_header">
                            <button type="button" class="btn btn-primary btn-sm" onclick="location.href='/Main/WithdrawRequestExcel';" style="float: right;"><i class="fa fa-plus"></i> Download Report</button>
                                            ...
                                            <tbody>
                                                <?php $i=1; ?>
                                                    ...
                                                    <tr style="color: #fff;">
                                                        <td>{{$i}}</td>
                                                        <td>{{$requests->uuid}}</td>
                                                        <td>{{$requests->usersname}}</td>
                                                        @if($requests->currency=='bank')
                                                        <td>{{$requests->accountname}} ({{$requests->address}})</td>
                                                        @else
                                                        <td>{{$requests->address}}</td>
                                                        @endif
                                                        <td>{{round($requests->amountsftc,2)}}</td>
                                                        <td>{{round($requests->amountusdt,2)}}</td>
                                                        <td>{{round($requests->deduction,2)}}</td>
                                                        <td>{{round($requests->net_amount,2)}}</td>
                                                        <td>{{$requests->currency}}</td>
                                                        <td>{{$requests->withdrawtype}}</td>
                                                        <td>{{$requests->txndate}}</td>
                                                        <td><span class="{{$requests->statusclass}}">{{$requests->status}}</span></td>
                                                        <td><a href="/Main/WithdrawApprove/{{$requests->txnid}}" class="text-danger" data-bs-toggle="tooltip" data-bs-original-title="Edit" target="_blank" style="color:#fff;"><i class="ti-pencil" aria-hidden="true"></i>Approve</a></td>
                                                    </tr>
                                                    <?php $i++; ?>
                                                    @endforeach                                         
                                            </tbody>
