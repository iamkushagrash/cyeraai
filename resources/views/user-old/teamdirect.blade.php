...
@include('user.sidebaruser')
		...
		<div id="content" class="app-content">
			...
			<h1 class="page-header">
				My Referral <!-- <small>page header description goes here...</small> -->
			</h1>
			...
			<!-- BEGIN #datatable -->
                          ...
                          @foreach($direct as $direct)
													<tr>
														...
														<td>{{$direct->name}}</td>
														<td>{{$direct->doj}}</td>
														<td>{{round($direct->shares)}}</td>
														<td>@if($direct->levelStatus()>=$direct->leveluser){{round($direct->levelStatus())}}@else{{round($direct->leveluser)}} @endif</td>
														<td>{{round($direct->teamtotal)}}</td>
														<td><span style="color:#{{$direct->statusclass}}">{{$direct->status}}</span></td>
													</tr>
													<?php $i++; ?>
                          @endforeach
												</tbody>
