...
@include('user.sidebaruser')
		...
		<div id="content" class="app-content">
			<ul class="breadcrumb">
				...
				<li class="breadcrumb-item active">Conversion History</li>
			</ul>
			
			<h1 class="page-header">
				Conversion History <!-- <br><small>Current Rank : </small> -->
			</h1>
			...
			<!-- BEGIN #datatable -->
								<div id="datatable" class="mb-5">
			                      ...
			                      <th>Status</th>
				                      ...
				                      <!-- <td>{{round($history->amountsftc,2)}}</td> -->
				                      <td>{{round($history->amountusdt,2)}}</td>
				                      <td>{{round($history->deduction,2)}}</td>
				                      <td>{{round($history->net_amount,2)}}</td>
															<td style="text-transform:uppercase;">{{$history->currency}}</td>
															<td>{{$history->created_at}}</td>
								              				<td class="text-end {{$history->statusclass}}">{{$history->status}}</td>
														</tr>
								            			<?php $i++; ?>
								            			@endforeach
												</tbody>
