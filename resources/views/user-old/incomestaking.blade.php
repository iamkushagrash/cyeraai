...
@include('user.sidebaruser')
		...
		<div id="content" class="app-content">
			<ul class="breadcrumb">
				...
				<li class="breadcrumb-item active">Staking Reward</li>
			</ul>
			
			<h1 class="page-header">
				Staking Reward <!-- <br><small>Current Rank : </small> -->
			</h1>
									...
									<div class="card">
										<div class="card-body">
							              ...
							              <th>Status</th>
												...
												<tbody>
													<?php $i=1; ?>
							            			@foreach($roiamount as $roiamount)
													<tr>
														<td>{{$i}}</td>
														<!-- <td><span style="color:#{{$roiamount->statusclass}}">{{round($roiamount->amount,2)}}</span></td> -->
														<td><span style="color:#{{$roiamount->statusclass}}">{{round($roiamount->amountusdt,2)}}</span></td>
														<td>{{round($roiamount->principal,2)}}</td>
														<td>{{round($roiamount->principalusdt,2)}}</td>
														<td>{{$roiamount->created_at}}</td>
							              <td>{{$roiamount->status}}</td>
													</tr>
							            			<?php $i++; ?>
							            			@endforeach
												</tbody>
