...
@include('user.sidebaruser')
		...
		<div id="content" class="app-content">
			...
			<h1 class="page-header">
				All Team <!-- <small>page header description goes here...</small> -->
			</h1>
									...
									<div class="card">
										<div class="card-body">
											<table id="datatableDefault" class="table text-nowrap w-100">
												...
												<tbody>
													<?php $i=1; ?>
                          @foreach($totaldown as $totaldown)
													<tr>
														<td>{{$i}}</td>
														<td>{{$totaldown->userid}}</td>
														<td>{{$totaldown->name}}</td>
														<td>{{$totaldown->doj}}</td>
														<td>{{$totaldown->level}}</td>
														<td>@if($totaldown->userDetails()->first()->levelStatus()>=$totaldown->leveluser){{round($totaldown->userDetails()->first()->levelStatus())}}@else{{round($totaldown->leveluser)}} @endif</td>
														<td>{{round($totaldown->current,2)}}</td>
														<td><span style="color:#{{$totaldown->statusclass}}">{{$totaldown->status}}</span></td>
													</tr>
													<?php $i++; ?>
                          @endforeach
												</tbody>
