...
@include('user.sidebaruser')
		...
		<div id="content" class="app-content">
			...
			<div class="row" style="margin-bottom: 1%;">
				<div class="col-md-2"><h4>Select Level</h4></div>
				<div class="col-md-3">
					<form method="get">
			...
			<!-- BEGIN #datatable -->
                          ...
                          @foreach($totaldown as $row)
													<tr>
														...
														<td>{{$row->doj}}</td>
														<td>{{$row->level}}</td>
														<td>@if($row->userDetails()->first()->levelStatus()>=$row->leveluser){{round($row->userDetails()->first()->levelStatus())}}@else{{round($row->leveluser)}} @endif</td>
														<td>{{round($row->current,2)}}</td>
														<td><span style="color:#{{$row->statusclass}}">{{$row->status}}</span></td>
													</tr>
													<?php $i++; ?>
                          @endforeach
												</tbody>
